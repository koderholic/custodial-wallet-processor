...
// 	_ = services.GetOnchainBalance(cache, s.Config, onchainBalanceRequest, &floatOnChainBalanceResponse, serviceErr)
// 	floatOnChainBalance, _ := new(big.Float).SetPrec(prec).SetString(floatOnChainBalanceResponse.Balance)

// 	// Get total users balance
// 	totalUserBalance, err := tasks.GetTotalUserBalance(baseRepository, floatAccount.AssetSymbol, userAssetRepository)
// 	if err != nil {
...
// 	toAddress, _, _ := tasks.GetSweepAddressAndMemo(cache, s.Config, baseRepository, floatAccount)

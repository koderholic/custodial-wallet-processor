...
func RegisterRoutes(router *mux.Router, validator *validation.Validate, config Config.Data, logger *utility.Logger, db *gorm.DB, memoryCache *utility.MemoryCache) {
	...
	once.Do(func() {
		...
		apiRouter.HandleFunc("/assets/process-batched-transactions", middlewares.NewMiddleware(logger, config, userAssetController.ProcessBatchBTCTransactions).LogAPIRequests().Build()).Methods(http.MethodPost)

...
func RegisterRoutes(router *mux.Router, validator *validation.Validate, config Config.Data, logger *utility.Logger, db *gorm.DB, memoryCache *utility.MemoryCache) {
	...
	once.Do(func() {
		...
		apiRouter.HandleFunc("/assets/{assetId}/create-auxiliary-address", middlewares.NewMiddleware(logger, config, userAssetController.CreateAuxiliaryAddress).ValidateAuthToken(utility.Permissions["ConfirmTransaction"]).LogAPIRequests().Build()).Methods(http.MethodPost)

...
func Up20200220214254(tx *sql.Tx) error {
	...
	_, err := tx.Exec("CREATE TABLE IF NOT EXISTS `denominations` (`id` varchar(36) NOT NULL, `created_at` timestamp NULL, `updated_at` timestamp NULL, `name` varchar(255), `asset_symbol` varchar(255) NOT NULL, `coin_type` BIGINT(20) NOT NULL, `decimal` int, `is_enabled` tinyint(1) DEFAULT 1, PRIMARY KEY (id), CONSTRAINT uix_denominations_symbol UNIQUE (symbol), INDEX isEnabled (is_enabled),INDEX symbol (symbol));")
	if err != nil {
		return err
	}
	return nil
